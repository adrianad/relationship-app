...
class _QuestionsViewState extends State<QuestionsView> {
  ...
  Preset get _currentPreset =>
      _presets.firstWhere((preset) => preset.name == _activePreset, orElse: () => _presets.first);
  ...
  Future<void> _generateQuestion() async {
    ...
    final defaultQuestion = questionProvider.getDefaultQuestion(languageProvider.currentLocale.languageCode);
    ...
    try {
      ...
      if (!questionProvider.anyMoodTone && _currentMoodTone != questionProvider.moodTone.first) {
        await questionProvider.updateSettings(anyMoodTone: false, moodTone: [_currentMoodTone]);
      }

      if (!questionProvider.anyCategory && _currentCategory != questionProvider.thematicCategory.first) {
        await questionProvider.updateSettings(anyCategory: false, thematicCategory: [_currentCategory]);
      }

      if (!questionProvider.anyGoal && _currentGoal != questionProvider.goalOfInteraction.first) {
        await questionProvider.updateSettings(anyGoal: false, goalOfInteraction: [_currentGoal]);
      }
  ...
  Future<void> _regenerateWithSameSettings() async {
    ...
    try {
      ...
      await questionProvider.regenerateWithSameSettings(language: languageProvider.currentLocale.languageCode);
  ...
  Future<void> _generateFunnyQuestion() async {
    ...
    try {
      ...
      await questionProvider.generateFunnyQuestion(language: languageProvider.currentLocale.languageCode);
  ...
  Future<void> _generateDeepQuestion() async {
    ...
    try {
      // Don't update profile settings, just use preset temporarily
      await questionProvider.generateDeepQuestion(language: languageProvider.currentLocale.languageCode);
  ...
  Future<void> _generateIcebreakerQuestion() async {
    ...
    try {
      // Don't update profile settings, just use preset temporarily
      await questionProvider.generateIcebreakerQuestion(language: languageProvider.currentLocale.languageCode);
  ...
  Future<void> _generateRandomQuestion() async {
    ...
    try {
      ...
      await questionProvider.generateRandomQuestion(language: languageProvider.currentLocale.languageCode);
  ...
  // We no longer need this custom chip since we're using ChoiceChip directly
                              ...
                              children: [
                                ...
                                ChoiceChip(
                                  label: Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: questionProvider.anyMoodTone ? Colors.white : Colors.grey.shade800,
                                      fontWeight: questionProvider.anyMoodTone ? FontWeight.bold : FontWeight.normal,
                                    ),
                                  ),
                                  avatar: Icon(
                                    Icons.shuffle,
                                    size: 14,
                                    color: questionProvider.anyMoodTone ? Colors.white : Colors.grey.shade600,
                                  ),
                                  selected: questionProvider.anyMoodTone,
                                  selectedColor: Colors.indigo.shade500,
                                  backgroundColor: Colors.grey.shade200,
                                  showCheckmark: false,
                                  elevation: questionProvider.anyMoodTone ? 3 : 0,
                                  onSelected: (selected) async {
                                    print('Tapped Random mood/tone');
                                    // Only change if it's not already selected
                                    if (!questionProvider.anyMoodTone) {
                                      // We need to explicitly include 'Random' in the list to trigger the random selection logic
                                      await questionProvider.updateSettings(anyMoodTone: true, moodTone: ['Random']);
                                      setState(() {
                                        _currentMoodTone = 'Random';
                                      });
                                    }
                                ...
                                ...questionProvider.moodOptions
                                    .map(
                                      (option) => ChoiceChip(
                                        label: Text(
                                          getLocalizedOption(context, option),
                                          style: TextStyle(
                                            fontSize: 12,
                                            color:
                                                (!questionProvider.anyMoodTone && _currentMoodTone == option)
                                                    ? Colors.white
                                                    : null,
                                            fontWeight:
                                                (!questionProvider.anyMoodTone && _currentMoodTone == option)
                                        ...
                                        showCheckmark: false,
                                        onSelected: (_) async {
                                          await questionProvider.updateSettings(anyMoodTone: false, moodTone: [option]);
                                          setState(() {
                              ...
                              children: [
                                ...
                                ChoiceChip(
                                  label: Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: questionProvider.anyCategory ? Colors.white : Colors.grey.shade800,
                                      fontWeight: questionProvider.anyCategory ? FontWeight.bold : FontWeight.normal,
                                    ),
                                  ),
                                  avatar: Icon(
                                    Icons.shuffle,
                                    size: 14,
                                    color: questionProvider.anyCategory ? Colors.white : Colors.grey.shade600,
                                  ),
                                  selected: questionProvider.anyCategory,
                                  selectedColor: Colors.indigo.shade500,
                                  backgroundColor: Colors.grey.shade200,
                                  showCheckmark: false,
                                  elevation: questionProvider.anyCategory ? 3 : 0,
                                  onSelected: (selected) async {
                                    print('Tapped Random category');
                                    // Only change if it's not already selected
                                    if (!questionProvider.anyCategory) {
                                      // We need to explicitly include 'Random' in the list to trigger the random selection logic
                                      ...
                                      (option) => ChoiceChip(
                                        label: Text(
                                          ...
                                          style: TextStyle(
                                            fontSize: 12,
                                            color:
                                                (!questionProvider.anyCategory && _currentCategory == option)
                                                    ? Colors.white
                                                    : null,
                                            fontWeight:
                                                (!questionProvider.anyCategory && _currentCategory == option)
                              ...
                              children: [
                                ...
                                ChoiceChip(
                                  label: Text(
                                    ...
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: questionProvider.anyGoal ? Colors.white : Colors.grey.shade800,
                                      fontWeight: questionProvider.anyGoal ? FontWeight.bold : FontWeight.normal,
                                    ),
                                  ),
                                  avatar: Icon(
                                    Icons.shuffle,
                                    size: 14,
                                    color: questionProvider.anyGoal ? Colors.white : Colors.grey.shade600,
                                  ),
                                  selected: questionProvider.anyGoal,
                                  selectedColor: Colors.indigo.shade500,
                                  backgroundColor: Colors.grey.shade200,
                                  showCheckmark: false,
                                  elevation: questionProvider.anyGoal ? 3 : 0,
                                  onSelected: (selected) async {
                                    print('Tapped Random goal');
                                    // Only change if it's not already selected
                                    if (!questionProvider.anyGoal) {
                                      // We need to explicitly include 'Random' in the list to trigger the random selection logic
                                      await questionProvider.updateSettings(
                                      ...
                                      (option) => ChoiceChip(
                                        label: Text(
                                          ...
                                          style: TextStyle(
                                            fontSize: 12,
                                            color:
                                                (!questionProvider.anyGoal && _currentGoal == option)
                                                    ? Colors.white
                                                    : null,
                                            fontWeight:
                                                (!questionProvider.anyGoal && _currentGoal == option)
                  ...
                  children: [
                    ...
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items:
                            questionProvider.contextOptions
                                .map(
                    ...
                    if (_showingTempSettings) ...[
                      if (_tempMoodTone.isNotEmpty)
                        Chip(
                          ...
                          labelStyle: TextStyle(
                            fontSize: 12,
                            color: _tempMoodTone == 'Random' ? Colors.white : null,
                            fontWeight: _tempMoodTone == 'Random' ? FontWeight.bold : FontWeight.normal,
                          ),
                          backgroundColor:
                              _tempMoodTone == 'Random' ? Colors.indigo.shade500 : Colors.amber.withOpacity(0.2),
                          elevation: _tempMoodTone == 'Random' ? 2 : 0,
                        ),
                      if (_tempCategory.isNotEmpty)
                        Chip(
                          label: Text(
                            _tempCategory == 'Random'
                          ...
                          labelStyle: TextStyle(
                            fontSize: 12,
                            color: _tempCategory == 'Random' ? Colors.white : null,
                            fontWeight: _tempCategory == 'Random' ? FontWeight.bold : FontWeight.normal,
                          ),
                          backgroundColor:
                              _tempCategory == 'Random' ? Colors.indigo.shade500 : Colors.amber.withOpacity(0.2),
                          elevation: _tempCategory == 'Random' ? 2 : 0,
                        ),
                      if (_tempDepth.isNotEmpty)
                        Chip(
                      ...
                      if (_tempGoal.isNotEmpty)
                        Chip(
                          ...
                          labelStyle: TextStyle(
                            fontSize: 12,
                            color: _tempGoal == 'Random' ? Colors.white : null,
                            fontWeight: _tempGoal == 'Random' ? FontWeight.bold : FontWeight.normal,
                          ),
                          backgroundColor:
                              _tempGoal == 'Random' ? Colors.indigo.shade500 : Colors.amber.withOpacity(0.2),
                          elevation: _tempGoal == 'Random' ? 2 : 0,
                        ),
                    ] else ...[
                      // Otherwise show the actual settings
                      Chip(
                        label: Text(
                          questionProvider.anyMoodTone
                              ? 'Random${questionProvider.lastRandomMoodTone != null ? ': ${getLocalizedOption(context, questionProvider.lastRandomMoodTone!)}' : ''}'
                        ...
                        backgroundColor: questionProvider.anyMoodTone ? Colors.indigo.shade500 : Colors.grey.shade200,
                        elevation: questionProvider.anyMoodTone ? 2 : 0,
                      ),
                      Chip(
                        label: Text(
                          questionProvider.anyCategory
                              ? 'Random${questionProvider.lastRandomCategory != null ? ': ${getLocalizedOption(context, questionProvider.lastRandomCategory!)}' : ''}'
                        ...
                        backgroundColor: questionProvider.anyCategory ? Colors.indigo.shade500 : Colors.grey.shade200,
                        elevation: questionProvider.anyCategory ? 2 : 0,
                      ),
                      Chip(
                        label: Text(
                          questionProvider.anyGoal
                              ? 'Random${questionProvider.lastRandomGoal != null ? ': ${getLocalizedOption(context, questionProvider.lastRandomGoal!)}' : ''}'
                        ...
                        backgroundColor: questionProvider.anyGoal ? Colors.indigo.shade500 : Colors.grey.shade200,
                        elevation: questionProvider.anyGoal ? 2 : 0,
                      ),
                    ],
                  ],
                ),
                        ...
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              // Vertical divider
                              Container(height: 28, width: 1, color: Colors.white.withOpacity(0.5)),
